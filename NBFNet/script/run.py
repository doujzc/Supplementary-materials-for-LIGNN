...
def train_and_validate(cfg, model, train_data, valid_data, filtered_data=None, test_data=None):
    ...
    for i in range(0, cfg.train.num_epoch, step):
        ...
        result = test(cfg, model, test_data, filtered_data=filtered_data)
        ...
        for metric in cfg.task.metric:
            if metric == "mr":
                ...
                if len(values) > 1:
                    ...
                    # score = 0
                    # for i in range(all_ranking.shape[0]):
                    #     index = torch.LongTensor(random.sample(range(all_num_negative[i]), 50))
                    #     all_ranking[i] = (index < (all_ranking[i]-1)).sum() + 1
