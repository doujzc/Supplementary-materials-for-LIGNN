...
class LILayer_ty(MessagePassing):
    def __init__(self, in_dim, hidden_dim, out_dim=None, bias=True, tol=1e-2):
        ...
        x0 = Parameter(torch.randn(in_dim, hidden_dim))
        W = Parameter(torch.randn(in_dim, hidden_dim, hidden_dim))
    ...
    def rescale(self):
        with torch.no_grad():
            self.W.weight /= self.W.weight.norm(dim=0)
...
class LIGNN_ty_sparse(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim=None, bias=True) -> None:
        super().__init__()
        self.in_dim = in_dim
        self.hidden_dim = hidden_dim
        self.out_dim = out_dim
    ...
    def forward_iteration(self, f, x0, max_iter, tol):
        f0 = f(x0)
        res = []
        for _ in range(max_iter):
            x = f0
    ...
    def forward(self, s, adj, N, max_iteration=5, contract=0.8, tol=1e-2):
